...
public class CartAction {
	...
	public Result addCartdetail(int fid, int count, HttpSession session) {
		...
		try {
			...
			e.printStackTrace();
			return Result.failure("系统错误，请联系管理员！", e.getMessage());
		} catch (BizException e) {
			e.printStackTrace();
			return Result.failure("添加购物车失败!", e.getMessage());
		}
		// 返回结果
	}
	...
	public List<?> queryCart(HttpSession session) {
		User user = (User) session.getAttribute("loginedUser");
		return cm.selectCart(user.getUid());
	}
	...
	public Result upCart(int fid, int count, HttpSession session) {
		...
		System.out.println("fid" + fid);
		System.out.println("count" + count);
		int i = cm.upCart(user.getUid(), fid, count);
		if(i > 0) {
	...
	public Result clearCart(HttpSession session) throws SQLException {
		User user = (User) session.getAttribute("loginedUser");
		int i = cm.clearCart(user.getUid());
		if(i > 0) {
			return Result.success("清空购物车成功！", null);
		}else {
			return Result.failure("清空购物车失败！", null);
		}
	}
	...
	public Result deleteCart(int fid,HttpSession session) {
		...
		int i = cm.deleteCart(fid);
		if(i > 0) {
			return Result.success("删除鲜花成功！", null);
		}else {
			return Result.failure("删除鲜花失败！", null);
		}
	}
}
